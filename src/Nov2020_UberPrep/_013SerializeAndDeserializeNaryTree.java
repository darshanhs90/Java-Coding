...
public class _013SerializeAndDeserializeNaryTree {
	...
	static class Codec {
		...
		public String serialize(Node root) {
			return serialiseHelper(root);
		}

		public String serialiseHelper(Node root) {
			if (root != null) {
				String output = root.val + ",";
				if (root.children != null) {
					output += root.children.size() + ",";
					for (int i = 0; i < root.children.size(); i++) {
						output += serialize(root.children.get(i));
					}
				} else {
					output += "0,";
				}
