...
public class _1335MinimumDifficultyOfAJobSchedule {
	...
	public static int minDifficulty(int[] jobDifficulty, int d) {
		...
		dfs(0, d, jobDifficulty, 0);
		return minSum;
	}

	public static void dfs(int startIndex, int d, int[] jobDifficulty, int currSum) {
		if (d == 0 && startIndex == jobDifficulty.length) {
		...
		int max = jobDifficulty[startIndex];
		for (int i = startIndex; i < jobDifficulty.length; i++) {
			max = Math.max(max, jobDifficulty[i]);
			dfs(i + 1, d - 1, jobDifficulty, currSum + max);
		}
	}
