...
public class _0269AlienDictionary {
	...
	public static String alienOrder(String[] words) {
		HashMap<Character, HashSet<Character>> wordMap = new HashMap<Character, HashSet<Character>>();
		HashMap<Character, Integer> countMap = new HashMap<Character, Integer>();
		for (int i = 0; i < words.length; i++) {
			String word = words[i];
			for (int j = 0; j < word.length(); j++) {
				wordMap.put(word.charAt(j), new HashSet<Character>());
		...
		for (int i = 0; i < words.length - 1; i++) {
			...
			for (int j = 0; j < Math.min(word1.length(), word2.length()); j++) {
				...
				if (c1 != c2) {
					...
					if (!wordMap.get(c1).contains(c2)) {
						wordMap.get(c1).add(c2);
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				char c = q.poll();
				sb.append(c);
				Iterator<Character> iter = wordMap.get(c).iterator();
