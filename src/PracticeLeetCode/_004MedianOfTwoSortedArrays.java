...
public class _004MedianOfTwoSortedArrays {
	...
	public static double findMedianSortedArrays(int[] nums1, int[] nums2) {
		...
		if(totalLength%2==0)
		{
			return (findKthElement(totalLength/2+1,nums1,nums2,0,0)+findKthElement(totalLength/2,nums1,nums2,0,0))/2.0;
		}
		else{
			return findKthElement(totalLength/2+1,nums1,nums2,0,0);
		}
	}
	private static int findKthElement(int k, int[] nums1, int[] nums2, int s1, int s2) {
		if(s1>=nums1.length)
			return nums2[s2+k-1];
		if(s2>=nums2.length)
			return nums2[s1+k-1];
